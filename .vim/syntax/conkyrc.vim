...
syn keyword ConkyrcSetting
      \ alignment append_file background border_inner_margin
      \ border_outer_margin border_width
      \ color0 color1 color2 color3 color4 color5 color6 color7 color8 color9 colorN
      ...
      \ diskio_avg_samples display double_buffer draw_borders draw_graph_borders
      \ draw_outline draw_shades extra_newline font format_human_readable gap_x gap_y
      \ if_up_strictness imap imlib_cache_flush_interval imlib_cache_size
      \ lua_draw_hook_post lua_draw_hook_pre lua_load lua_shutdown_hook lua_startup_hook
      \ mail_spool max_port_monitor_connections max_specials max_text_width
      \ max_user_text maximum_width minimum_size mpd_host mpd_password mpd_port
      \ music_player_interval net_avg_samples no_buffers nvidia_display out_to_console
...
syn keyword ConkyrcVarName contained nextgroup=ConkyrcNumber,ConkyrcColour
      ...
      \ battery_short battery_time blink bmpx_album bmpx_artist bmpx_bitrate
      \ bmpx_title bmpx_track bmpx_uri buffers cached cmdline_to_pid
      \ color color0 color1 color2 color3 color4 color5 color6 color7 color8 color9
      \ combine conky_build_arch conky_build_date conky_version cpu cpubar cpugauge
      \ cpugraph curl desktop desktop_name desktop_number disk_protect diskio
      \ diskio_read diskio_write diskiograph diskiograph_read diskiograph_write
      ...
      \ execgraph execi execibar execigauge execigraph execp execpi flagged_mails
      \ font format_time forwarded_mails freq freq_g fs_bar fs_bar_free fs_free
      \ fs_free_perc fs_size fs_type fs_used fs_used_perc goto gw_iface gw_ip
      \ hddtemp head hr hwmon i2c i8k_ac_status i8k_bios i8k_buttons_status
      \ i8k_cpu_temp i8k_left_fan_rpm i8k_left_fan_status i8k_right_fan_rpm
      ...
      \ loadavg loadgraph lua lua_bar lua_gauge lua_graph lua_parse machine mails
      \ mboxscan mem membar memeasyfree memfree memgauge memgraph memmax memperc
      \ mixer mixerbar mixerl mixerlbar mixerr mixerrbar moc_album moc_artist
      \ moc_bitrate moc_curtime moc_file moc_rate moc_song moc_state moc_timeleft
      \ moc_title moc_totaltime monitor monitor_number mpd_album mpd_artist mpd_bar
      \ mpd_bitrate mpd_elapsed mpd_file mpd_length mpd_name mpd_percent mpd_random
      \ mpd_repeat mpd_smart mpd_status mpd_title mpd_track mpd_vol nameserver
      \ new_mails nodename nodename_short nvidia obsd_product obsd_sensors_fan
      \ obsd_sensors_temp obsd_sensors_volt obsd_vendor offset outlinecolor
      \ pb_battery pid_chroot pid_cmdline pid_cwd pid_environ pid_environ_list
      \ pid_exe pid_nice pid_openfiles pid_parent pid_priority pid_state
      \ pid_state_short pid_stderr pid_stdin pid_stdout pid_threads pid_thread_list
      \ pid_time_kernelmode pid_time_usermode pid_time pid_uid pid_euid pid_suid
      \ pid_fsuid pid_gid pid_egid pid_sgid pid_fsgid pid_read pid_vmpeak pid_vmsize
      \ pid_vmlck pid_vmhwm pid_vmrss pid_vmdata pid_vmstk pid_vmexe pid_vmlib
      \ pid_vmpte pid_write platform pop3_unseen pop3_used pre_exec processes
      \ read_tcp replied_mails rss running_processes running_threads scroll
      \ seen_mails shadecolor smapi smapi_bat_bar smapi_bat_perc smapi_bat_power
      \ smapi_bat_temp sony_fanspeed stippled_hr swap swapbar swapfree swapmax
      \ swapperc sysname tab tail tcp_portmon template0 template1 template2
      \ template3 template4 template5 template6 template7 template8 template9
      \ texeci threads time to_bytes top top_io top_mem top_time totaldown totalup
      ...
      \ xmms2_bar xmms2_bitrate xmms2_comment xmms2_date xmms2_duration xmms2_elapsed
      \ xmms2_genre xmms2_id xmms2_percent xmms2_playlist xmms2_size xmms2_smart
